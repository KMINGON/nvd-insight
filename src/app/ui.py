...
try:
    ...
    from ..analytics.charts import cwe as cwe_chart
    from ..analytics.charts import cvss_app
...
def render_published_trend_page(
    ...
    with analysis_tab:
        # 전체 기간에 포함된 CVE 레코드 수를 간단한 metric으로 보여준다.
        ...
        # 연도별 추이, 월별 추이, Heatmap을 한 화면에 배치해 계절성을 비교한다.
    ...
    with ai_tab:
        # RAG 인덱스를 사용할 수 없을 때는 즉시 안내한다.
        if retriever is None:
            st.info("RAG 검색기를 사용할 수 없습니다. 인덱스 상태를 확인하세요.")
            return
        # 월별 포커스 연도가 있으면 해당 연도의 월별 요약으로 RAG 컨텍스트를 교체한다.
...
def render_cvss_page(
    ...
    analysis_tab, ai_tab = st.tabs(["분석 시각화", "AI 요약 리포트"])
    with analysis_tab:
        # 데이터가 어느 CVSS 버전에 얼마나 채워졌는지 먼저 표로 보여준다.
        st.markdown("**CVSS 데이터 존재 여부**")
        st.dataframe(summary_df)

        # v3.1/v2 중 어떤 메트릭을 기반으로 차트를 그릴지 사용자가 선택한다.
        ...
        # baseScore 구간을 직접 커스터마이징해 조직별 위험도 컷을 시험할 수 있게 한다.
        ...
        col1, col2 = st.columns(2)
        with col1:
            # baseSeverity 분포를 시각화하고 동일 데이터를 챗봇 입력용 집계로도 저장한다.
            try:
        ...
        with col2:
            # 선택한 bins 기준으로 baseScore 히스토그램을 그리고 챗봇용 테이블을 준비한다.
            try:
    ...
    with ai_tab:
        # RAG 검색기를 사용할 수 없는 경우 사용자에게 바로 안내한다.
        if retriever is None:
            st.info("RAG 검색기를 사용할 수 없습니다. 인덱스 상태를 확인하세요.")
            return
        # 가용성/심각도/점수 구간 테이블을 하나로 합쳐 챗봇 컨텍스트를 구성한다.
...
def render_cwe_page(
    ...
    with analysis_tab:
        # 사용자가 Top-N 범위를 조절해 시각화/표를 즉시 업데이트할 수 있게 한다.
...
INSIGHT_PAGES = {
    ...
    "cvss": InsightPage(
        key="cvss",
        label="CVSS 분포 인사이트",
        description="CVSS v3/v2 데이터의 존재 여부, 심각도·점수 분포를 한 번에 확인합니다.",
        render=render_cvss_page,
    ),
    "cwe": InsightPage(
        key="cwe",
        label="CWE Top-N 인사이트",
        description="자주 등장하는 CWE 유형을 시각화하고 대응 우선순위를 제안합니다.",
        render=render_cwe_page,
    ),
}
